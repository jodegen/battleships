...
import { connect, createWsApp, flushRedis, HAS_INFRA, registerCookie, sleep, type WsContext } from './setup-ws';

describe.skipIf(!HAS_INFRA)('Quick Play: Disconnect entfernt still aus der Queue (006, US2/FR-013)', () => {
  let ctx: WsContext;
  beforeAll(async () => {
  ...
  it('entfernt einen wartenden Spieler bei Disconnect — keine Partie, kein Statistik-Eintrag', async () => {
    ...
    const user = await ctx.prisma.user.findUnique({ where: { email: 'wait@x.com' }, include: { stat: true } });
