...
import { createWsApp, flushRedis, HAS_INFRA, quickMatch, waitFor, type WsContext } from './setup-ws';
...
describe.skipIf(!HAS_INFRA)('Quick Play: gematchte Partie verläuft identisch zur Code-Lobby (006, US1/FR-007)', () => {
  let ctx: WsContext;
  beforeAll(async () => {
  ...
  it('verwendet Standard-Einstellungen, spielt zu Ende, reconnectet per Token und persistiert Stats für beide', async () => {
    ...
    const { a, b, code, am, bm } = await quickMatch(ctx, { host: 'alice@x.com', guest: 'bob@x.com' });

    // Lobby-Sicht trägt die Standard-Einstellungen (FR-005, SC-003).
    const aState = await waitFor<{ settings: typeof QUICK_PLAY_SETTINGS; status: string }>(a, 'lobby:state', 50).catch(
      () => null,
    );
    ...
    const resume = (await b.emitWithAck('reconnect:resume', { code, token: bm.reconnectToken })) as {
      ok: boolean;
      you?: string;
    };
    ...
    for (const target of bCells) {
      const ack = (await a.emitWithAck('shot:fire', { code, moveId: nextMoveId(), target })) as { ok: boolean };
    ...
    const bob = await ctx.prisma.user.findUnique({ where: { email: 'bob@x.com' }, include: { stat: true } });
